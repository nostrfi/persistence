...
public class Events
{
    ...
    ///     0 - Metadata : Content is set to stringified JSON object containing metadata about the feed, describing who created
    ///     it, what it is for, etc.
    ///     relays may delete older events once it gets a new one for the same PubKey.
    ///     1 - Text Note :  the content is set to the plaintext content of a note. Content that must be parsed, such as
    ///     Markdown and HTML, should not be used. Clients should also not parse content as those.
    /// </summary>
